...
class CodeBertClassifier:
    ...
    def train(
        ...
        print_status(f"[*] Using device: {self.device}")
        ...
        print_status(f"[+] Training samples: {len(train_texts)}")
        print_status(f"[+] Validation samples: {len(val_texts)}")

        # Create datasets
        print_status("[*] Creating datasets and dataloaders...")
        ...
        print_status(f"[*] Number of service labels: {service_num_labels}")
        print_status(f"[*] Number of activity labels: {activity_num_labels}")
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, batch in enumerate(train_loader, 1):
                if batch_idx % 10 == 0:  # Print progress every 10 batches
                    ...
                    print_status(f"[*] Training progress: {progress:.1f}% - Batch {batch_idx}/{batch_count}")
                ...
                service_loss = service_criterion(service_pred, service_labels)
                activity_loss = activity_criterion(activity_pred, activity_labels)
            ...
            print_status(f"[+] Average training loss: {avg_train_loss:.4f}")
            ...
            with torch.no_grad():
                for batch in val_loader:
                    ...
                    val_service_preds.extend(torch.argmax(service_pred, dim=1).cpu().numpy())
                    val_activity_preds.extend(torch.argmax(activity_pred, dim=1).cpu().numpy())
                    val_service_true.extend(batch['service_label'].numpy())
                    val_activity_true.extend(batch['activity_label'].numpy())

            # Calculate accuracies
            service_accuracy = np.mean(np.array(val_service_preds) == np.array(val_service_true))
            activity_accuracy = np.mean(np.array(val_activity_preds) == np.array(val_activity_true))

            print_status(f"[+] Validation Results - Epoch {epoch + 1}:")
            print_status(f"[*] Service Accuracy: {service_accuracy:.4f}")
            print_status(f"[*] Activity Accuracy: {activity_accuracy:.4f}")
